...
viz = Visdom(env='PiaNet without GRU Server13.41')
...
def train_valid_seg():
	...
	for epoch in range(max_epoches):
		for epi in range(episode):
			...
			train_loss, train_liver, train_tumor, train_iter = train(train_loader, net, loss, optimizer)
			...
			valid_loss, valid_liver, valid_tumor, valid_iter = validate(val_loader, net, loss, epoch, episode, saved_dir)
			valid_avgloss = sum(valid_loss) / valid_iter
			valid_avglilver = sum(valid_liver) / valid_iter
			valid_avgtumor = sum(valid_tumor) / valid_iter
			print("[%d-%d/%d], valid_loss:%.4f,, valid_liver:%.4f, valid_tumor:%.4f, Time:%.3fmin " %
				  (epoch, epi, max_epoches, valid_avgloss, valid_avglilver, valid_avgtumor, (time.time() - start_time) / 60))
...
def train(data_loader, net, loss, optimizer):
	...
	return epoch_loss,epoch_liver,epoch_tumor,total_iter
