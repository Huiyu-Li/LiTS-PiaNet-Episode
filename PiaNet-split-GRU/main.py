...
viz = Visdom(env='PiaNet split GRU Server13.41')
viz.line([0], [0], win='loss-dice')
viz.line([0], [0], win='loss-dice-gru')
...
def train_valid_seg():
	...
	max_epoches = 2
	episode = 2
	train_batch_size = 2; valid_batch_size = 1; test_batch_size = 1
	channels = 1; depth = 16; height = 48; width = 48
	learning_rate = 0.01
	weight_decay = 1e-4
	config = {
		# 'model':'USNETres',
		'train_csv_list': ['./TVTcsv/train' + str(i) + '.csv' for i in range(episode)],
		'valid_csv': './TVTcsv/valid.csv',
		'test_csv': './test.csv',
		# below is the saved path
		'ckpt_dir': './results/',
		'saved_dir': "/media/data/LITS/segResults/",
		'model_dir': "./results/20190813/model_0.pth"
		# "./results/20190818/model_0.pth"
		# "./results/20190817/model_1.pth"
		# './results/20190813/model_1.pth'
	}
	##########hyperparameters##########
	# refresh save dir
	...
	for epoch in range(max_epoches):
		for epi in range(episode):
			...
			(train_loss, train_liver, train_tumor,
			 train_loss_gru, train_liver_gru, train_tumor_gru, train_iter) = train(train_loader, net, gru, loss, optimizer, optimizer_gru)
			...
			train_avgliver = sum(train_liver) / train_iter
			train_avgtumor = sum(train_tumor) / train_iter
			train_avgloss_gru = sum(train_loss_gru) / train_iter
			train_avgliver_gru = sum(train_liver_gru) / train_iter
			train_avgtumor_gru = sum(train_tumor_gru) / train_iter

			print("[%d-%d/%d], train_loss:%.4f, train_liver:%.4f, train_tumor:%.4f, Time:%.3fmin" %
				(epoch, epi, max_epoches-1, train_avgloss, train_avgliver, train_avgtumor, (time.time() - start_time) / 60))
			print("gru----, train_loss:%.4f, train_liver:%.4f, train_tumor:%.4f" %
				(train_avgloss_gru, train_avgliver_gru, train_avgtumor_gru))

			print('######valid epoch-epi', str(epoch), '-', str(epi), '######')
			(valid_loss, valid_liver, valid_tumor,
			 valid_loss_gru, valid_liver_gru, valid_tumor_gru, valid_iter) = validate(val_loader, net, gru, loss, epoch, episode, saved_dir)

			valid_avgloss = sum(valid_loss) / valid_iter
			valid_avgliver = sum(valid_liver) / valid_iter
			valid_avgtumor = sum(valid_tumor) / valid_iter
			valid_avgloss_gru = sum(valid_loss_gru) / valid_iter
			valid_avgliver_gru = sum(valid_liver_gru) / valid_iter
			valid_avgtumor_gru = sum(valid_tumor_gru) / valid_iter

			print("[%d-%d/%d], valid_loss:%.4f,, valid_liver:%.4f, valid_tumor:%.4f, Time:%.3fmin " %
				 (epoch, epi, max_epoches, valid_avgloss, valid_avgliver, valid_avgtumor, (time.time() - start_time) / 60))
			print("gru----, valid_loss:%.4f, valid_liver:%.4f, valid_tumor:%.4f" %
				 (valid_avgloss_gru, valid_avgliver_gru, valid_avgtumor_gru))
			...
			if min_validloss > abs(valid_avgloss):
				...
				state = {
					'epoche': epoch,
					'arch': str(net),'arch_gru': str(gru),
					'state_dict': net.state_dict(),'state_dict_gru': gru.state_dict(),
					'optimizer': optimizer.state_dict(),'optimizer_gru': optimizer_gru.state_dict(),
					# other measures
				}
				torch.save(state, ckpt_dir + '/checkpoint.pth.tar')
				# save model
				#PiaNet and #if str(episode) the former one will be ovrrided
				model_filename = ckpt_dir + '/model_' + str(epoch)+ '-' + str(epi) + '.pth'
				torch.save(net.state_dict(), model_filename)
				print('Model saved in', model_filename)
				#Gru
				model_filename_gru = ckpt_dir + '/model_' + str(epoch) + '-' + str(epi) + '_gru.pth'
				torch.save(gru.state_dict(), model_filename_gru)
				print('Model saved in', model_filename_gru)
			viz.line([train_avgloss], [epoch * 10 + epi], win='train', opts=dict(title='train avgloss'),
					 update='append')
			viz.line([valid_avgloss], [epoch * 10 + epi], win='valid', opts=dict(title='valid avgloss'),
					 update='append')
...
def train(data_loader, net, gru, loss, optimizer, optimizer_gru):
	...
	for i, (data,target,origin,direction,space,ct_name) in enumerate(data_loader):
		...
		epoch_liver_gru.append(liver_dice_gru)
		epoch_tumor_gru.append(tumor_dice_gru)
	...
	viz.line(Y=np.column_stack((epoch_loss_gru, epoch_liver_gru, epoch_tumor_gru)),
			 X=np.column_stack((x, x, x)), win='loss-dice-gru',
			 opts=dict(legend=['loss', 'liver', 'tumor']), update='new')
	return (epoch_loss, epoch_liver, epoch_tumor,
			epoch_loss_gru, epoch_liver_gru, epoch_tumor_gru,total_iter)

def validate(data_loader, net, gru, loss, epoch, episode, saved_dir):
	net.eval()
	gru.eval()
	...
	with torch.no_grad():#no backward
		for i, (data,target,origin,direction,space,ct_name) in enumerate(data_loader):
			...
			epoch_loss_gru.append(loss_output_gru.item())  # Use tensor.item() to convert a 0-dim tensor to a Python number
			epoch_liver_gru.append(liver_dice)
			epoch_tumor_gru.append(tumor_dice)
			print("gru----loss:%.4f, liver_dice:%.4f, tumor_dice:%.4f" % (loss_output_gru.item(), liver_dice_gru, tumor_dice_gru))

			if epoch % episode == 0:
				#PiaNet
				output1_name = os.path.join(saved_dir, 'valid-' + ct_name[0] + '-' + str(epoch) + '-' + str(episode) + '-output1' + '.nii')
				...
				#GRU
				output1_name_gru = os.path.join(saved_dir, 'valid-' + ct_name[0] + '-' + str(epoch) + '-' + str(episode) + '-output1_gru' + '.nii')
				output2_name_gru = os.path.join(saved_dir, 'valid-' + ct_name[0] + '-' + str(epoch) + '-' + str(episode) + '-output2_gru' + '.nii')
